...
static void AddRoundKey(uint8_t round,state_t* state,uint8_t* RoundKey)
{
	...
	#pragma HLS ARRAY_RESHAPE variable=RoundKey cyclic factor=16 dim=0
    uint8_t i, j, c = (round*Nb)<<2;
...
static void SubBytes(state_t* state)
{
	...
	SubBytes_label35:for (i = 0; i < 4; ++i)
	{
		...
		{
			#pragma HLS unroll
			uint8_t val = (*state)[j][i];
			(*state)[j][i] = getSBoxValue(val);
		}
	}
}
...
void AES_CBC_decrypt_buffer(struct AES_ctx* ctx, uint8_t* buf,  uint32_t length)
{
	...
	uint8_t storeActInp[AES_BLOCKLEN];
	for (i = 0; i < length; i += AES_BLOCKLEN)
	{
		memcpy(storeNextIv, buf, AES_BLOCKLEN);
		memcpy(storeActInp, buf, AES_BLOCKLEN);
		InvCipher((state_t*)storeActInp, ctx->RoundKey);
		XorWithIv(storeActInp, ctx->Iv);
		memcpy(buf, storeActInp, AES_BLOCKLEN);
